...
class VLETimeLineStateModel {
    ...
    var isVideoAudioMuted: Bool = false
    ...
    var isHaveRenderTrack: Bool {
        if renderTrackItemModelArray.isEmpty
            && separateRenderTrackItemModelArray.isEmpty
        {
            return false
        } else {
            return true
    ...
    public var renderTrackItemModelArray: [VLETimeLineItemModel] {
        ...
        set {
            ...
            if _renderTrackItemModelArray.isEmpty
                && _separateRenderTrackItemModelArray.isEmpty
            {
    ...
    public func fetchScaleSpaceWidth() -> CGFloat {
        return CGFloat.init(VLETimeLineConfig.framesPerSpace)
            * VLETimeLineConfig.ptPerFrames
    }
    ...
    public func fetchScaleViewWidth() -> CGFloat {
        ...
        print(
            "🔍 Scale width calculation - Main track: \(mainTrackSeconds)s, Total: \(totalSeconds)s"
        )

        // ✅ Use main track duration for timeline scale
        return CGFloat(
            mainTrackSeconds * Float64(VLETimeLineConfig.framesPerSecond))
            * VLETimeLineConfig.ptPerFrames
    }
    ...
    private func calculateScaleWidth() -> CGFloat {
        return CGFloat.init(totalSeconds * VLETimeLineConfig.framesPerSecond)
            * VLETimeLineConfig.ptPerFrames
    }

    // ✅ ADD NEW COMPUTED PROPERTIES
    var isValidComposition: Bool {
        return !renderTrackItemModelArray.isEmpty
            || !separateRenderTrackItemModelArray.isEmpty
    }
    ...
    func validateComposition() -> (isValid: Bool, issues: [String]) {
        ...
        for i in 1..<renderTrackItemModelArray.count {
            ...
            let prevEnd = CMTimeAdd(
                prev.globalStartTime, prev.source.selectedTimeRange.duration)

            if current.globalStartTime < prevEnd {
                issues.append(
                    "Time overlap detected between segments \(i-1) and \(i)")
            }
        }
    ...
    public func renderTrackItemModelConvertToSeparate(
        at selectIndex: Int, startTime: CMTime
    ) {
        print("🔍 === SEPARATE OPERATION DEBUG ===")
        print(
            "🔍 Before separate - Main track count: \(renderTrackItemModelArray.count)"
        )
        ...
        print(
            "🔍 Original main duration: \(CMTimeGetSeconds(originalMainDuration))s"
        )
        ...
        if let pendingTime = pendingOverlayStartTime {
            ...
            print(
                "🕐 Using user-selected time: \(CMTimeGetSeconds(requestedTime))s"
            )
        } else {
        ...
        let overlayDurationSeconds = CMTimeGetSeconds(
            selectedModel.source.selectedTimeRange.duration)
        ...
        if requestedSeconds >= originalMainSeconds {
            finalStartTime = CMTime(
                seconds: max(0, originalMainSeconds - overlayDurationSeconds),
                preferredTimescale: 600)
            print(
                "⚠️ Overlay time outside original timeline, corrected to: \(CMTimeGetSeconds(finalStartTime))s"
            )
        } else if (requestedSeconds + overlayDurationSeconds)
            > originalMainSeconds
        {
            finalStartTime = CMTime(
                seconds: originalMainSeconds - overlayDurationSeconds,
                preferredTimescale: 600)
            print(
                "⚠️ Overlay extends beyond original timeline, corrected to: \(CMTimeGetSeconds(finalStartTime))s"
            )
        } else {
            finalStartTime = requestedTime
            print(
                "✅ Overlay timing valid: \(CMTimeGetSeconds(finalStartTime))s")
        }

        // ✅ Configure overlay item
        selectedModel.globalStartTime = finalStartTime
        selectedModel.isSeparateRenderTrack = true
        selectedModel.renderLayer.timeRange = CMTimeRange(
            start: finalStartTime,
            duration: selectedModel.source.selectedTimeRange.duration)
        ...
        let transform = Transform(
            center: center, rotation: 0, scale: overlayScale)
        ...
        print(
            "📊 Final state - Main: \(renderTrackItemModelArray.count), Overlay: \(separateRenderTrackItemModelArray.count)"
        )
    ...
    private func calculateOptimalOverlayScale(
        for itemModel: VLETimeLineItemModel
    ) -> Float {
        // ✅ Use pending scale if available
        if let pendingScale = pendingOverlayScale {
        ...
        print(
            "🎯 Calculating scale for type: \(itemType), duration: \(sourceDuration)s"
        )
    ...
    public func clipSeparateRenderTrackItemModelAtCurrentIndex(
        clipRate rate: Float,
        completion: @escaping (NSError?, VLETimeLineItemModel?) -> Void
    ) {
        guard let itemModel = currentSelectedItemModel else {
            return
        }
        let selectedDuration = self.calculateSelectedTime(
            at: Float(rate),
            sourceTime: itemModel.source.selectedTimeRange.duration)
        let newItemModel = VLETimeLineItemModel.init(
            with: itemModel.source, type: itemModel.type)
        newItemModel.isSeparateRenderTrack = true
        newItemModel.source = itemModel.source.copy()
        newItemModel.source.load { error in
            if error == nil {
                newItemModel.globalStartTime = CMTimeAdd(
                    itemModel.globalStartTime, selectedDuration)
                newItemModel.source.selectedTimeRange.start = CMTimeAdd(
                    itemModel.source.selectedTimeRange.start, selectedDuration)
                newItemModel.source.selectedTimeRange.duration = CMTimeSubtract(
                    itemModel.source.selectedTimeRange.duration,
                    selectedDuration)
                newItemModel.renderLayer.timeRange = CMTimeRange.init(
                    start: newItemModel.globalStartTime,
                    duration: newItemModel.source.selectedTimeRange.duration)
                itemModel.source.selectedTimeRange.duration = selectedDuration
                newItemModel.generateThumbnails(with: 1) { _ in }
                self.separateRenderTrackItemModelArray.insert(
                    newItemModel, at: self.currentSelectedIndex! + 1)
    ...
    public func calculateSelectedTime(at rate: Float, sourceTime: CMTime)
        -> CMTime
    {
    ...
    public func clipRenderTrackItemModelAtCurrentIndex(
        clipRate rate: Float, completion: @escaping (NSError?) -> Void
    ) {
        guard let itemModel = currentSelectedItemModel else {
        ...
        print(
            "✂️ Original duration: \(CMTimeGetSeconds(itemModel.source.selectedTimeRange.duration))s"
        )

        let selectedDuration = calculateSelectedTime(
            at: Float(rate),
            sourceTime: itemModel.source.selectedTimeRange.duration)
        ...
        newItemModel.source.load { [weak self] error in
            ...
            if error == nil {
                ...
                newItemModel.source.selectedTimeRange.start = CMTimeAdd(
                    itemModel.source.selectedTimeRange.start, selectedDuration)
                newItemModel.source.selectedTimeRange.duration =
                    remainingDuration
                ...
                print(
                    "✅ Original item updated: duration=\(CMTimeGetSeconds(selectedDuration))s"
                )
                print(
                    "✅ New item created: start=\(CMTimeGetSeconds(newItemModel.globalStartTime))s, duration=\(CMTimeGetSeconds(remainingDuration))s"
                )
                ...
                itemModel.generateThumbnails(
                    with: max(1, Int(CMTimeGetSeconds(selectedDuration) / 2))
                ) { _ in
                ...
                thumbnailGroup.enter()
                newItemModel.generateThumbnails(
                    with: max(1, Int(CMTimeGetSeconds(remainingDuration) / 2))
                ) { _ in
                ...
                thumbnailGroup.notify(queue: .main) {
                    ...
                    self.renderTrackItemModelArray.insert(
                        newItemModel, at: self.currentSelectedIndex! + 1)
    ...
    public func refreshItemTime() {
        ...
        for item in renderTrackItemModelArray {
            ...
            item.renderLayer.timeRange = CMTimeRange(
                start: mainTrackDuration,
                duration: item.source.selectedTimeRange.duration)
            mainTrackDuration = CMTimeAdd(
                mainTrackDuration, item.source.selectedTimeRange.duration)
        }
        ...
        for item in separateRenderTrackItemModelArray {
            ...
            if CMTimeCompare(overlayEnd, mainTrackDuration) > 0 {
                ...
                let correctedDuration = CMTimeSubtract(
                    mainTrackDuration, overlayStart)
            ...
            maxOverlayEnd = CMTimeMaximum(
                maxOverlayEnd,
                CMTimeAdd(overlayStart, item.source.selectedTimeRange.duration))
        }

        // ✅ 3. Set total duration to main track duration (not including overlay extensions)
        self.totalDuration = mainTrackDuration  // ← KEY FIX
        self.totalSeconds = VLETimeLineConfig.convertToSecond(
            value: mainTrackDuration)
    ...
    func calculateMainTrackDuration() -> CMTime {
        ...
        for item in renderTrackItemModelArray {
            totalDuration = CMTimeAdd(
                totalDuration, item.source.selectedTimeRange.duration)
        }

        print(
            "🔍 Main track duration calculated: \(CMTimeGetSeconds(totalDuration))s"
        )
    ...
    func validateTimelineConsistency() -> Bool {
        ...
        for (index, item) in renderTrackItemModelArray.enumerated() {
            if item.globalStartTime != expectedStart {
                print(
                    "❌ Gap detected at index \(index): expected \(CMTimeGetSeconds(expectedStart))s, got \(CMTimeGetSeconds(item.globalStartTime))s"
                )
                return false
            }
            expectedStart = CMTimeAdd(
                expectedStart, item.source.selectedTimeRange.duration)
        }
        ...
        for (index, item) in separateRenderTrackItemModelArray.enumerated() {
            ...
            if endSeconds > mainDurationSeconds {
                ...
                if maxAllowedDuration > 0.5 {
                    item.source.selectedTimeRange.duration = CMTime(
                        seconds: maxAllowedDuration, preferredTimescale: 600)
                    item.renderLayer.timeRange.duration =
                        item.source.selectedTimeRange.duration
                    print(
                        "✂️ Trimmed overlay \(index) to duration: \(maxAllowedDuration)s"
                    )
                } else {
                    print(
                        "❌ Overlay \(index) too close to end, should be removed"
                    )
                }
            }
        }
    ...
    func resetAudioMixingState() {
        isVideoAudioMuted = false
        print("🔄 Audio mixing state reset to default")
    }

    func validateAudioState() -> Bool {
        let hasVideoAudio = renderTrackItemModelArray.contains { $0.type == .video }
        let hasAddedAudio = separateRenderTrackItemModelArray.contains { $0.type == .audio }
        
        // ✅ If no added audio exists, unmute video audio
        if !hasAddedAudio && isVideoAudioMuted {
            print("⚠️ No added audio found, unmuting video audio")
            isVideoAudioMuted = false
            return false
        }
    ...
    func toggleVideoAudioMute() -> Bool {
        // ✅ Validate state first
        let wasValid = validateAudioState()
        
        // ✅ Only toggle if we have both video and added audio
        let hasVideoAudio = renderTrackItemModelArray.contains { $0.type == .video }
        let hasAddedAudio = separateRenderTrackItemModelArray.contains { $0.type == .audio }
        
        guard hasVideoAudio && hasAddedAudio else {
            print("⚠️ Cannot toggle: missing video(\(hasVideoAudio)) or audio(\(hasAddedAudio))")
            isVideoAudioMuted = false // Force unmuted if missing components
            return false
        }
        
        isVideoAudioMuted.toggle()
        print("🎵 Audio toggle: videoMuted=\(isVideoAudioMuted)")
        
        // ✅ Apply immediately
        VLEMainConcreteMediator.shared.refreshAudioMixing()
        
        return isVideoAudioMuted
    }

    func removeAudioTrack(at index: Int) {
        guard index < separateRenderTrackItemModelArray.count else { return }
        
        let removedItem = separateRenderTrackItemModelArray[index]
        if removedItem.type == .audio {
            separateRenderTrackItemModelArray.remove(at: index)
            
            // ✅ Reset audio state when audio is removed
            let hasRemainingAudio = separateRenderTrackItemModelArray.contains { $0.type == .audio }
            if !hasRemainingAudio {
                print("🗑️ Last audio track removed, resetting audio state")
                resetAudioMixingState()
            }
    ...
    func getAudioMixingInfo() -> (
        hasVideoAudio: Bool, hasAddedAudio: Bool, videoMuted: Bool
    ) {
        let hasVideoAudio = renderTrackItemModelArray.contains {
            $0.type == .video
        }
        let hasAddedAudio = separateRenderTrackItemModelArray.contains {
            $0.type == .audio
        }

        return (hasVideoAudio, hasAddedAudio, isVideoAudioMuted)
    }
}
